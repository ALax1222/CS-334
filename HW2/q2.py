...
def holdout(model, xFeat, y, testSize):
    ...
    metrics.roc_curve(y_train, pred_train)
    trainAuc = metrics.auc(fpr_train, tpr_train)

    fpr_test, tpr_test, thresholds = metrics.roc_curve(y_test, pred_test)
    metrics.roc_curve(y_test, pred_test)
    testAuc = metrics.auc(fpr_test, tpr_test)
...
def kfold_cv(model, xFeat, y, k):
    ...
    for train_index, test_index in kf.split(xFeat, y):
        ...
        fpr_train, tpr_train, thresholds = metrics.roc_curve(y_train, pred_train)
        metrics.roc_curve(y_train, pred_train)
        trainAuc.append(metrics.auc(fpr_train, tpr_train))
    ...
    return sum(trainAuc)/len(trainAuc), sum(testAuc)/len(testAuc), timeElapsed
...
def mc_cv(model, xFeat, y, testSize, s):
    ...
    for i in range(s):
        train, test, x = (holdout(model, xFeat, y, testSize))
        trainAuc.append(train)
        testAuc.append(test)
